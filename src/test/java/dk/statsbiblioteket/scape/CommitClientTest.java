...
public class CommitClientTest {
    ...
    public void setUp() throws Exception {
        ...
        e = new IntellectualEntity.Builder().identifier(new Identifier(testIdentifier)).lifecycleState(
                new LifecycleState(
                        "", LifecycleState.State.NEW)).representations(
                Arrays.asList(
                        new Representation.Builder().identifier(
                                new Identifier("Representation")).title("representation").technical(
                                "SCAPE_REPRESENTATION_TECHNICAL",
                                createTextMDRecord()).files(
                                Arrays.asList(
                                        new eu.scape_project.model.File.Builder().technical(
                                                "JPYLYZER", createTextMDRecord()).identifier(
                                                new Identifier(
                                                        "The_One_File")
                                        ).build()
                                )
